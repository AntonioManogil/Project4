...
class RemindersActivityTest :
    ...
    @get:Rule
    var locationRule = GrantPermissionRule.grant(android.Manifest.permission.ACCESS_FINE_LOCATION)!!
    ...
    @Test
    fun shouldSave_Reminder() = runBlocking {
        val activityScenario = ActivityScenario.launch(RemindersActivity::class.java)
        dataBindingIdlingResource.monitorActivity(activityScenario)

        val myActivity = getActivity(activityScenario)
        ...
        // Testing toast
        myActivity?.let {
            onView(withText(R.string.reminder_saved)).inRoot(withDecorView(not(`is`(myActivity.window.decorView))))
                .check(
                    matches(
                        isDisplayed()
                    )
    ...
    // From https://alexzh.com/ui-testing-of-android-runtime-permissions/
